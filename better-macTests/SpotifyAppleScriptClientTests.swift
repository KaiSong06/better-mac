...
final class SpotifyAppleScriptClientTests: XCTestCase {
    func testParsesPlayingTrack() {
        let raw = "Sugar\tMaroon 5\tV\t235000\t12.5\thttps://i.scdn.co/image/abc\tplaying"
    ...
    func testParsesPausedTrack() {
        let raw = "Sugar\tMaroon 5\tV\t235000\t12.5\thttps://i.scdn.co/image/abc\tpaused"
        let track = SpotifyAppleScriptClient.parse(output: raw)
        XCTAssertNotNil(track)
        XCTAssertEqual(track?.name, "Sugar")
    ...
    func testEmptyOutputMeansStopped() {
    ...
    func testElapsedIsClampedToDuration() {
        let raw = "A\tB\tC\t5000\t99\thttps://example.com\tplaying"
        let track = SpotifyAppleScriptClient.parse(output: raw)
        XCTAssertEqual(track?.duration, 5.0)
        XCTAssertEqual(track?.elapsed ?? -1, 5.0, accuracy: 0.001)
    }

    func testMissingStateFieldDefaultsToPlaying() {
        // Backwards compat with any callers that don't include the trailing
        // state field — older output should still parse as playing.
        let raw = "A\tB\tC\t5000\t12\thttps://example.com"
        let track = SpotifyAppleScriptClient.parse(output: raw)
        XCTAssertNotNil(track)
        XCTAssertTrue(track?.isPlaying ?? false)
    }
